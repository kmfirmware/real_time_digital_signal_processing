...
%	
clear;
fftL=360;                     % Length of fft for calculate the spectrum
frame_sel=6;
frame=180;                    % LPC frame size
[b,fs]=audioread('voice4.wav',[frame*(frame_sel-1)+1,frame*frame_sel],'native');
...
figure('Name',strcat('Different Gammas LPC order ',num2str(lpcOrder)),'NumberTitle','off');
plot(f,pyy(1:(fftL/2)),'-',f,pyy_1(1:(fftL/2)),'-.',f,pyy_2(1:(fftL/2)),'--.',f,pyy_3(1:(fftL/2)),'--');

xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');
title('A(Z) and W(Z) filter spectrum responses');
h = legend('LPC envelope','\gamma2=0.50&\gamma1=1.0','\gamma2=0.25&\gamma1=1.0','\gamma2=0.10&\gamma1=1.0',4); 
